...
test_cuda_toolkit_package() {
    ...
    echo "Checking: cuda-toolkit-12-9 package is installed"
    if ! rpm -q cuda-toolkit-12-9 >/dev/null 2>&1; then
        fail "cuda-toolkit-12-9 package is not installed"
    fi
    pass "cuda-toolkit-12-9 package is installed"

    if [[ $VERBOSE -eq 1 ]]; then
        verbose_log "Package: $(rpm -q cuda-toolkit-12-9)"
    fi
...
test_cuda_libraries() {
    ...
    local libs=("libcudart.so" "libcublas.so" "libcufft.so" "libcurand.so")
    local found=0
...
main() {
    ...
    if [[ ! -d /usr/local/cuda ]] && ! rpm -q cuda-toolkit-12-9 >/dev/null 2>&1; then
        echo ""
        log "=========================================================="
